...
def _clean_lines(lines, filepath):
    ...
    for ii, line in enumerate(lines):
        ...
        line = line.replace(PATH_IMAGES_FOLDER, op.join(
            path_rel_root_one_up, 'images'))
...
if __name__ == '__main__':
    ...
    PATH_TEMPLATE = op.join(PATH_SITE_ROOT, 'assets',
    ...
    for ix_file, page in enumerate(tqdm(list(toc))):
        ...
        yaml_fm += ['prev_page:']
        yaml_fm += ['  url: {}'.format(
            _prepare_url(url_prev_page).replace('"', "'"))]
        yaml_fm += ["  title: '{}'".format(prev_file_title)]
        yaml_fm += ['next_page:']
        yaml_fm += ['  url: {}'.format(
            _prepare_url(url_next_page).replace('"', "'"))]
